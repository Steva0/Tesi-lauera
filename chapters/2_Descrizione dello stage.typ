...
#text(style: "italic", [
    In questo capitolo descrivo l'organizzazione del lavoro durante il tirocinio, l'ambiente di sviluppo utilizzato, gli strumenti adottati e l'approccio metodologico seguito.
])
#v(1em)
...
*Visual Studio Code* come editor di testo principale, utilizzato sia per la scrittura e modifica del codice sorgente CPL di RVC, sia per la stesura della documentazione in Typst. L'editor è stato configurato con estensioni per la sintassi CPL e per la compilazione live dei documenti Typst.

*GitHub* per il versionamento della tesi e dei file di lavoro personali. Il repository della relazione finale è ospitato su GitHub con pubblicazione automatica del PDF tramite _GitHub Actions_ e _GitHub Pages_.
...
*Terminale Windows* (cmd e PowerShell) per l'esecuzione dei comandi RVC, la gestione dei file e le operazioni di debug. La maggior parte delle operazioni con RVC avviene tramite interfaccia a riga di comando.

*PuTTY* come client SSH per Windows, utilizzato per la gestione delle chiavi SSH tramite il componente `puttygen`.

*ssh-keygen* (OpenSSH per Windows) per la generazione delle chiavi Ed25519, la firma crittografica dei file e la verifica delle firme SSH. Questo strumento è centrale nel meccanismo di sicurezza implementato in RVC.

*AGE* come strumento di cifratura moderno, studiato durante lo stage in preparazione alla fase di progettazione delle _repository_ cifrate prevista negli obiettivi desiderabili.
...
Nella *fase iniziale* l'attenzione era rivolta allo studio delle tecnologie crittografiche — chiavi SSH, firma digitale, AGE — attraverso documentazione ufficiale, esempi pratici al terminale e confronto con il tutor aziendale. Parallelamente ho iniziato a esplorare RVC dall'esterno, analizzandone il comportamento tramite i comandi disponibili e studiando il formato dei file prodotti.

Una volta ottenuto accesso ai *sorgenti CPL*, l'approccio è cambiato: dall'analisi esterna (_black-box_) si è passati all'analisi interna (_white-box_), con lettura sistematica del codice, identificazione delle vulnerabilità e progettazione degli interventi migliorativi. Questa fase ha richiesto anche lo studio del linguaggio CPL, per il quale non esiste documentazione pubblica — la comprensione è avvenuta tramite lettura del codice esistente e consultazione della documentazione interna fornita dall'azienda.
...
- Una *directory di lavoro* (`simulazione/`) contenente i file di un progetto fittizio su cui eseguire le operazioni RVC.
- Una *repository locale* (`repo/`) dove vengono archiviati i _commit_, separata dalla directory di lavoro.
- Un file `.git/repository.info` che collega la directory di lavoro alla repository, seguendo la convenzione di RVC.
...
Questo ambiente ha permesso di simulare scenari realistici — inclusi la manomissione dei file di _commit_, la compromissione delle chiavi SSH e la verifica della propagazione degli errori nella catena degli hash — senza rischiare di danneggiare dati reali.
