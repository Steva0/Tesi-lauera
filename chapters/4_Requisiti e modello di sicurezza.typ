...
Per mitigare parzialmente questa limitazione, il modello proposto prevede che l'identificativo di ogni commit sia composto da due componenti: il timestamp codificato in base36 per mantenere l'ordinamento cronologico visivo, e una porzione dell'hash del contenuto per garantire l'unicità crittografica. Un identificativo nella forma `0Q6JTD7XVZ_A3F2B1C4` — dove la prima parte è il timestamp e la seconda sono i primi otto caratteri dell'hash SHA256 dell'archivio ZIP — rende praticamente impossibile la collisione intenzionale mantenendo la leggibilità e l'ordinamento per nome file. Questa modifica è identificata come requisito nel modello ideale e discussa nell'analisi del divario nella @sec:analisi-divario.
...
- `id`: identificativo del commit, composto da timestamp in base36 e #gl("hash") parziale del contenuto — ad esempio `0Q6JTD7XVZ_A3F2B1C4`.
