...
Il responsabile aggiunge o rimuove dipendenti dal proprio progetto in autonomia tramite #gl("commit") amministrative — modificando il file `allowed_Dipendenti` e firmando la #gl("commit") con la propria chiave-privata. L'amministratore può sovrascrivere il file `allowed_Dipendenti` di qualsiasi progetto in qualsiasi momento — il suo potere non è vincolato dalla struttura gerarchica. Se un responsabile lascia l'azienda o viene rimosso dal ruolo, l'amministratore nomina un sostituto aggiornando il file `allowed_Responsabili`. Fino alla nomina del sostituto il progetto entra in stato di attesa: i dipendenti esistenti possono continuare a committare, ma non è possibile aggiungere nuovi dipendenti né modificare i permessi esistenti.
...
Il dipendente è autorizzato a produrre #gl("commit") ordinarie su un progetto se e solo se la propria chiave-pubblica è presente nel campo `allowed_signers` della #gl("commit") più recente di quel progetto. I permessi sono definiti per progetto — un dipendente autorizzato su ProgettoA non ha accesso a ProgettoB, anche se entrambi appartengono alla stessa #gl("repository"). Non esistono permessi per #gl("branch"): un dipendente autorizzato su un progetto può committare su qualsiasi #gl("branch") di quel progetto.

I dipendenti non possono produrre #gl("commit") amministrative — qualsiasi tentativo di creare, modificare o eliminare un file speciale (`allowed_Dipendenti`, `.rvc_policy`, `.rvc_branch_status`) viene rifiutato dal motore indipendentemente dalla presenza della firma. Questa restrizione vale per tutti i livelli di sicurezza.
...
+ Viene creata la primissima #gl("commit") del progetto `_rvc_root`. Questa #gl("commit") è fondamentale perché inizializza lo stato del motore e deve contenere:
...
+ Questa prima #gl("commit") viene firmata con la chiave master stessa, stabilendo l'ancora di fiducia interna al sistema. 
...
+ Il motore di #gl("rvc", capitalize: true) riceve la #gl("commit"). Poiché la chiave master non è elencata in `allowed_Dipendenti`, il motore procederebbe a rifiutarla. Tuttavia, prima di emettere il rifiuto definitivo, il motore verifica la firma della #gl("commit") contro il file `master.pub` registrato in modo immutabile nella #gl("commit") iniziale di `_rvc_root`. Se la firma combacia, il motore riconosce l'autorità suprema della chiave master e accetta la #gl("commit"); altrimenti la rifiuta.

=== Inizializzazione di un progetto

La creazione di un nuovo progetto all'interno di una #gl("repository") esistente segue percorsi diversi a seconda del livello di sicurezza scelto. La prima #gl("commit") di qualsiasi progetto è sempre una #gl("commit") amministrativa, ma i file da generare e i soggetti autorizzati cambiano in base al contesto.
...
+ Il responsabile produce la prima #gl("commit") del progetto, firmata con la propria chiave-privata. Il motore accetta la #gl("commit") verificando che il firmatario sia in `_rvc_root` e si sia auto-incluso nel nuovo `allowed_Dipendenti`.

Per i *progetti ai livelli di sicurezza 0 e 1*, non esistendo il file `allowed_Dipendenti` né la figura del responsabile, l'inizializzazione può essere eseguita esclusivamente dall'amministratore. L'amministratore produce una prima #gl("commit") contenente unicamente il file `.rvc_policy` (che dichiara il livello 0 o 1) e la firma con la propria chiave operativa. Qualsiasi tentativo da parte di un responsabile o di un dipendente di inizializzare un progetto a questi livelli viene rifiutato dal motore.

Il livello di sicurezza definito nella prima #gl("commit") non può essere abbassato — può essere alzato in qualsiasi momento tramite una #gl("commit") amministrativa firmata dal responsabile o dall'amministratore. Questa scelta elimina la possibilità di degradare le garanzie di sicurezza di un progetto già avviato.
...
Il file `.sig` è il punto di contatto tra il contenuto crittografico e il modello di sicurezza. Nel modello proposto la sua struttura estende quella attuale di #gl("rvc", capitalize: true) con i campi necessari per supportare la gerarchia di fiducia, i livelli di sicurezza configurabili e la gestione dei #gl("branch"). Il `.sig` è firmato crittograficamente nella sua interezza — qualsiasi modifica a uno qualsiasi dei suoi campi invalida la firma e quindi la #gl("commit").

I campi del `.sig` nel modello proposto sono i seguenti:

- `author`: identificativo dell'autore della #gl("commit"), nella forma definita dal file `allowed_Dipendenti` del progetto. Il formato — email, nome opaco o qualsiasi altra convenzione — è una scelta dell'organizzazione che gestisce la #gl("repository").
- `comment`: messaggio descrittivo della #gl("commit").
- `fn`: nome del file ZIP corrispondente.
- `id`: identificativo della #gl("commit"), composto da timestamp in base36 e #gl("hash") parziale del contenuto — ad esempio `0Q6JTD7XVZ_A3F2B1C4`.
- `prevId`: identificativo della #gl("commit") precedente.
- `hash`: sha256 del file ZIP di questa #gl("commit").
- `prevHash`: sha256 del file ZIP della #gl("commit") precedente.
- `cumulativeHash`: sha256 della concatenazione dell'hash attuale con il `cumulativeHash` della #gl("commit") precedente.
...
+ La firma della #gl("commit") viene verificata crittograficamente contro le chiavi pubbliche presenti nel campo `allowed_signers` del `.sig` di quella #gl("commit"). Il campo `allowed_signers` è fidato perché il motore garantisce che solo il responsabile o l'amministratore possano produrre le #gl("commit") amministrative che lo modificano — qualsiasi altra modifica viene rifiutata prima della creazione della #gl("commit"). Di conseguenza, se una #gl("commit") esiste ed è crittograficamente valida, il suo campo `allowed_signers` riflette una lista di autorizzati approvata da chi ne aveva il potere.
...
Nessuna firma è richiesta per le #gl("commit") ordinarie. Le #gl("commit") amministrative (come la prima #gl("commit") di inizializzazione prodotta dall'amministratore) costituiscono un'eccezione architetturale globale: devono sempre essere firmate e includere il campo `allowed_signers` nel `.sig`. Per le #gl("commit") ordinarie, invece, chiunque abbia accesso fisico alla #gl("repository") può aggiungere #gl("commit"). Il sistema non impone la verifica automatica dell'identità né dell'integrità, sebbene quest'ultima resti calcolabile matematicamente tramite la catena degli #gl("hash"). Il file `.sig` per le #gl("commit") ordinarie al livello 0 contiene solo i campi strutturali — `author`, `comment`, `fn`, `id`, `prevId`, `hash`, `prevHash`, `cumulativeHash`, `security_level` e `branch_status` — ma non il campo `allowed_signers` e non la firma #gl("ssh", capitalize: true). L'assenza della firma nelle #gl("commit") ordinarie è la caratteristica distintiva del livello 0 e viene rilevata dal motore come indicazione che il progetto opera senza controllo delle identità.
...
Nei progetti a livello 4 il file `allowed_Dipendenti` risiede all'interno dello ZIP cifrato — è parte del contenuto riservato e non è accessibile a chi non ha i permessi di lettura. La verifica crittografica rimane comunque possibile per qualsiasi osservatore grazie al campo `allowed_signers` presente in chiaro nel `.sig`: questo campo contiene le chiavi pubbliche degli autorizzati al momento della #gl("commit") ed è parte del contenuto firmato, quindi la sua integrità è garantita dalla firma stessa. Un osservatore senza permessi di lettura può verificare che la #gl("commit") sia firmata da una chiave presente negli `allowed_signers` del `.sig`, risalire alla gerarchia tramite `_rvc_root` e verificare l'intera catena di fiducia — senza mai dover decifrare il contenuto del progetto.
...
+ Il responsabile produce una #gl("commit") amministrativa firmata con la modifica al file `allowed_Dipendenti` — il motore verifica che la firma appartenga al responsabile o all'amministratore prima di accettarla.
...
+ Questa #gl("commit") di revoca viene firmata eccezionalmente con la *chiave-privata master*.
+ Il motore di #gl("rvc", capitalize: true) riceve la #gl("commit"). Poiché il firmatario non è in `allowed_Dipendenti` prima di rifutare la #gl("commit") verifica la firma contro il file `master.pub` registrato in modo immutabile nella #gl("commit") iniziale di `_rvc_root`. Se la firma combacia, il motore riconosce l'autorità suprema della chiave master, accetta la #gl("commit") e rende operativa la nuova delega altriemnti rifiuta la #gl("commit").
...
I #gl("branch") sono uno strumento fondamentale nello sviluppo software parallelo — permettono di isolare funzionalità, correzioni e sperimentazioni senza interferire con il lavoro principale. In un sistema di versionamento sicuro la gestione dei #gl("branch") introduce scenari che vanno affrontati esplicitamente: un #gl("branch") può diventare obsoleto, può essere abbandonato o può risultare compromesso a seguito di #gl("commit") non autorizzate.

Il principio fondamentale che governa la gestione dei #gl("branch") nel modello proposto è l'*immutabilità della storia*: nessuna #gl("commit") viene mai cancellata o modificata retroattivamente. Qualsiasi intervento su un #gl("branch") — archiviazione, chiusura o dichiarazione di compromissione — avviene aggiungendo nuove #gl("commit") che ne attestano lo stato, non rimuovendo quelle esistenti. Questo principio garantisce che la storia del progetto rimanga sempre verificabile nella sua interezza, inclusa la traccia degli eventi straordinari.

=== Archiviazione di un branch 

Un #gl("branch") di sviluppo che ha concluso il proprio ciclo di vita — perché la funzionalità è stata integrata nel #gl("branch") principale o perché è stata abbandonata — può essere marcato come archiviato tramite una #gl("commit") amministrativa. Il responsabile produce una #gl("commit") firmata sul #gl("branch") contenente il file speciale `.rvc_branch_status` dentro lo ZIP, che dichiara lo stato `archived` e può includere motivazione, riferimenti e note aggiuntive. Lo stato viene estratto da questo file e riportato nel campo `branch_status` del `.sig` — in chiaro e parte del contenuto firmato. Il motore legge esclusivamente il campo `branch_status` del `.sig` per determinare lo stato del #gl("branch"), senza dover accedere allo ZIP — questo garantisce il corretto funzionamento a qualsiasi livello di sicurezza, incluso il livello 4 con contenuto cifrato.

I #gl("branch") archiviati sono trattati dal motore in una speciale modalità protetta: rifiutano tassativamente qualsiasi nuova #gl("commit") ordinaria, cristallizzando di fatto lo stato del codice. È possibile leggerne la storia in modo completo, ma i dipendenti non possono aggiungervi modifiche.
L'archiviazione è un'operazione reversibile esclusivamente tramite una #gl("commit") amministrativa con la modifica del file `.rvc_branch_status` per impostarlo ad `active`, con il conseguente aggiornamento del campo `branch_status` nel `.sig`. Questa #gl("commit") riporta il #gl("branch") allo stato operativo normale e ristabilisce i permessi di scrittura standard.

=== Chiusura di branch compromessi

Un #gl("branch") compromesso è un #gl("branch") su cui sono state prodotte una o più #gl("commit") fraudolente o non autorizzate — ad esempio durante la finestra di rischio successiva alla compromissione di una chiave-privata. La gestione di questo scenario segue una procedura in due fasi.

Nella prima fase il #gl("branch") compromesso viene dichiarato tale tramite una #gl("commit") amministrativa firmata dal responsabile o dall'amministratore. Il file `.rvc_branch_status` dentro lo ZIP dichiara lo stato `compromised`, l'identificativo della prima #gl("commit") sospetta, la motivazione e qualsiasi informazione aggiuntiva utile alla gestione dell'incidente. Lo stato `compromised` viene estratto e riportato nel campo `branch_status` del `.sig` — il motore legge questo campo direttamente e blocca immediatamente il #gl("branch") senza dover decifrare il contenuto, indipendentemente dal livello di sicurezza del progetto. Le #gl("commit") fraudolente rimangono nella storia e sono visibili, ma il #gl("branch") è marcato come non affidabile.

Nella seconda fase viene creato un nuovo #gl("branch") pulito a partire dall'ultima #gl("commit") verificata come integra prima della compromissione. Lo sviluppo riprende sul nuovo #gl("branch"). Il #gl("branch") compromesso rimane nella #gl("repository") come evidenza dell'incidente — la sua storia è verificabile e costituisce la prova crittografica di cosa è accaduto e quando.
...
Questa scelta è deliberata: introdurre permessi per #gl("branch") aggiungerebbe complessità gestionale significativa senza un corrispondente aumento delle garanzie di sicurezza. La sicurezza del sistema si basa sull'autenticità delle #gl("commit"), non sulla restrizione dei #gl("branch") su cui è possibile scrivere. Un dipendente che produce una #gl("commit") non autorizzata — ad esempio direttamente sul #gl("branch") principale saltando il processo di verifica — è comunque identificabile tramite la firma crittografica e la sua azione è permanentemente tracciata nella storia del progetto.
