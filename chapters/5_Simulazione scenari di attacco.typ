...
I test sono stati condotti su una #gl("repository") locale composta da cinque #gl("commit") legittimi firmati con chiave #gl("ssh", capitalize: true), ripristinata allo stato iniziale prima di ogni tecnica tramite una copia di backup. La struttura dei #gl("commit") dello stato iniziale è la seguente:
...
L'attaccante produce un #gl("commit") tramite il motore #gl("rvc", capitalize: true) senza fornire una chiave #gl("ssh", capitalize: true). Nella versione iniziale la firma è opzionale e il motore non la richiede.
...
*Risultato.* Il motore accetta il #gl("commit") senza nessun avviso. Il verificatore segnala `[WARN] firma:ASSENTE` ma non incrementa il contatore degli errori critici. Tutte le operazioni successive del motore procedono normalmente.

*Impatto: Alto.* L'attaccante può inserire qualsiasi modifica al codice sorgente senza che il motore lo rilevi. Il #gl("commit") fraudolento è visibile nella history con un autore vuoto ma non produce nessun segnale automatico di anomalia — è rilevabile solo tramite esecuzione esplicita del verificatore.

*Requisiti violati:* RS06 — autenticità e non ripudio.

=== T2 — Commit con identità falsa

L'attaccante produce un #gl("commit") senza firma dichiarando nel campo `author` il nome di un autore autorizzato. Poiché non esiste una firma crittografica, il campo `author` non è verificabile e può contenere qualsiasi valore.
...
*Risultato.* Il motore segnala che la chiave privata non è stata trovata ma accetta comunque il #gl("commit") senza firma. Il verificatore produce lo stesso risultato di T1 — `[WARN] firma:ASSENTE` con il nome dell'autore dichiarato. Un osservatore che legge la history vede il #gl("commit") attribuito a Michele senza nessun indicatore di anomalia.
...
*Impatto: Alto.* L'attaccante può impersonare qualsiasi autore autorizzato producendo un #gl("commit") che nella history risulta indistinguibile da uno legittimo. Il rischio aumenta significativamente in contesti dove la firma non è considerata obbligatoria nella pratica operativa.
...
*Risultato.* Il verificatore rileva immediatamente la discrepanza tra l'hash del file ZIP modificato e quello dichiarato nel `.sig`. Il #gl("commit") alterato produce errori a cascata su tutti i successivi. Il motore continua a funzionare normalmente senza rilevare l'alterazione.

*Impatto: Basso.* La catena degli hash è già una garanzia operativa nella versione iniziale — la modifica è rilevabile in modo inequivocabile. La protezione è però solo reattiva: il motore non blocca l'alterazione, la rileva solo il verificatore su richiesta esplicita.

*Requisiti violati:* RS01, RS02 — integrità e ordine verificabile.

=== T4 — Sostituzione completa di ZIP e .sig

L'attaccante sostituisce sia il file ZIP che il `.sig` di un #gl("commit") esistente con valori ricalcolati su contenuto alterato, nel tentativo di produrre un #gl("commit") che superi la verifica dell'hash.
...
*Risultato.* Il #gl("commit") alterato supera la verifica dell'hash — il ricalcolo corretto inganna il verificatore sul nodo modificato. La catena si rompe nel #gl("commit") successivo. Applicando il ricalcolo in cascata su tutti i #gl("commit") successivi è possibile eliminare gli errori critici riducendo l'output a soli warning.
...
L'attaccante tenta di inserire un #gl("commit") tra due #gl("commit") esistenti modificando i riferimenti `prevId` e `prevHash` e ricalcolando la catena degli hash in cascata. Il risultato è analogo a T4 — i #gl("commit") modificati producono warning per la firma assente e i #gl("commit") successivi producono errori di catena fino al completamento del ricalcolo in cascata.

*Impatto: Alto.* Le implicazioni sono identiche a T4 — senza firme obbligatorie è possibile inserire #gl("commit") arbitrari in qualsiasi punto della storia riducendo l'output a soli warning. La firma #gl("ssh", capitalize: true) è la contromisura necessaria: qualsiasi modifica al `.sig` originale invalida la firma e produce un errore critico.

*Requisiti violati:* RS02, RS03 — integrità e ordine verificabile.

=== T6 — Replay di un commit legittimo

L'attaccante copia un #gl("commit") legittimo esistente e lo reintroduce nella #gl("repository") con un nuovo nome file.
...
*Risultato.* Il verificatore rileva immediatamente l'errore di catena. La history mostra il #gl("commit") reintrodotto come un branch separato. Il ricalcolo del `cumulativeHash` è possibile ma non permette di alterare il contenuto dello ZIP — la firma originale diventerebbe invalida.

*Impatto: Basso.* L'attaccante non può inserire codice arbitrario — può solo duplicare #gl("commit") esistenti creando branch con lo stesso nome, sporcando la history senza modificarne il contenuto.

*Requisiti violati:* RS02, RS03 — ordine verificabile.

=== T7 — Modifica del .sig senza modificare lo ZIP

L'attaccante modifica solo il file `.sig` di un #gl("commit") firmato lasciando lo ZIP intatto e la firma originale invariata.
...
*Risultato.* Qualsiasi modifica a qualsiasi campo del `.sig` invalida la firma #gl("ssh", capitalize: true), che copre il file nella sua interezza. L'errore è localizzato al solo #gl("commit") modificato — i successivi rimangono validi perché il `cumulativeHash` non è stato alterato.

*Impatto: Nullo.* La firma #gl("ssh", capitalize: true) è una protezione già operativa nella versione iniziale — qualsiasi tentativo di alterare i metadati di un #gl("commit") firmato è immediatamente rilevabile.
...
L'attaccante elimina fisicamente uno o più #gl("commit") dalla #gl("repository") cancellando i file ZIP e `.sig` corrispondenti.
...
*Risultato.* Il verificatore rileva la discontinuità nella catena. La history mostra la catena spezzata come due branch separati senza connessione.

*Impatto: Nullo.* Il troncamento è immediatamente rilevabile e non produce nessun vantaggio per l'attaccante. I #gl("commit") successivi al punto di eliminazione producono errori critici e la history risulta incoerente.

*Requisiti violati:* RS02 — ordine verificabile.

=== T9 — Commit con cumulativeHash falsificato

L'attaccante produce un #gl("commit") con hash e `cumulativeHash` calcolati correttamente sul nuovo ZIP ma con `prevHash` che punta a un #gl("commit") inesistente.
...
*Risultato.* Il verificatore rileva l'errore di catena sul #gl("commit") anomalo lasciando validi tutti gli altri. Il #gl("commit") risulta isolato e non collegato alla storia legittima.

*Impatto: Basso.* L'attaccante non riesce a inserire un #gl("commit") che si agganci alla storia legittima senza produrre errori critici. Il #gl("commit") falsificato è immediatamente identificabile come anomalo e isolato dalla catena principale.

*Requisiti violati:* RS02, RS03 — integrità e ordine verificabile.

*Nota.* I commit successivi risultano OK perchè non sono collegati al commit falsificato, che è isolato come un branch separato.

=== T10 — Iniezione di una repository fasulla

L'attaccante crea da zero una #gl("repository") completa con una catena di #gl("commit") senza firma, strutturalmente valida dal punto di vista degli hash, e la distribuisce come se fosse legittima.
...
#figure(caption: "Sintesi dei risultati — Scenario 1.")[
  #table(
    ...
    [T1],  [#gl("commit", capitalize: true) senza firma],         [WARN],     [No], [Alto],
    ...
    [T6],  [Replay #gl("commit") legittimo],                     [ERR],      [No], [Basso],
...
Il risultato più significativo dello scenario 1 è la distinzione tra due categorie di tecniche. Le tecniche T3, T6, T7, T8 e T9 producono errori critici rilevabili dal verificatore — la catena degli hash e la firma #gl("ssh", capitalize: true) forniscono già protezione efficace, con impatto nullo o basso. Le tecniche T1, T2, T4, T5 e T10 producono solo warning — l'attaccante può agire senza produrre errori critici, con impatto alto in tutti i casi. Il denominatore comune è l'assenza di firma #gl("ssh", capitalize: true) obbligatoria. T4 e T5 meritano attenzione particolare: con ricalcolo in cascata degli hash è possibile riscrivere la storia di un progetto producendo solo warning, rendendo l'attacco praticamente invisibile a chi non esegue il verificatore in modo sistematico.

== Scenario 2 — Dipendente con chiave SSH valida ma non autorizzato

Il secondo scenario simula un dipendente dell'organizzazione che possiede una chiave #gl("ssh", capitalize: true) valida e sa produrre #gl("commit") firmati correttamente, ma non è autorizzato a operare su uno o più progetti specifici. A differenza dello scenario 1, i #gl("commit") prodotti sono crittograficamente validi — la firma è presente e corretta. Il verificatore deve quindi distinguere tra "firma valida" e "firmatario autorizzato", due proprietà che nella versione iniziale non sono entrambe verificate preventivamente dal motore.

=== T1 — Commit firmato da chiave non autorizzata

Il dipendente firma un #gl("commit") con la propria chiave #gl("ssh", capitalize: true) valida su un progetto a cui non dovrebbe avere accesso. Il motore non verifica se la chiave del firmatario sia presente in una lista di autorizzati — nella versione iniziale questa lista non esiste.
...
*Risultato.* Il motore accetta il #gl("commit") senza nessun avviso e le operazioni successive procedono normalmente. Il verificatore rileva il problema segnalando `[ERR] firma:FALLITA` — la chiave di Luigi non è presente nel file `allowed_signers`. L'anomalia è rilevabile solo tramite verifica esplicita.

*Impatto: Alto.* Un dipendente non autorizzato può produrre #gl("commit") su qualsiasi progetto senza che il motore lo impedisca. Il #gl("commit") è crittograficamente firmato e appare nella history come un #gl("commit") normale — la differenza rispetto a uno legittimo emerge solo dal verificatore. In assenza di verifiche periodiche, l'accesso non autorizzato può passare inosservato a lungo.
...
Il dipendente firma il #gl("commit") con la propria chiave #gl("ssh", capitalize: true) ma dichiara nel campo `author` il nome di un collega autorizzato. Il motore accetta il #gl("commit") perché la firma è crittograficamente valida. Il verificatore rileva la discrepanza tra l'autore dichiarato e la chiave effettivamente usata per la firma.
...
*Impatto: Alto.* Il rischio principale esiste nei contesti dove la verifica delle firme non è eseguita sistematicamente ma basta la presenza di una qualsiasi firma. In quel caso il #gl("commit") appare firmato da un autore autorizzato — Michele — e passa inosservato. Solo il verificatore con il file `allowed_signers` corretto rivela che la chiave usata non appartiene all'autore dichiarato.

*Requisiti violati:* RS07, RS08 — autenticità e autorizzazione.

=== T3 — Volume di commit non autorizzati senza rilevamento operativo

Il dipendente produce una serie di #gl("commit") firmati con la propria chiave su un progetto non autorizzato nel corso del tempo. L'obiettivo è dimostrare che il motore non emette nessun segnale operativo durante le operazioni normali, indipendentemente dal numero di #gl("commit") non autorizzati presenti.
...
*Risultato.* Il motore esegue tutti i #gl("commit") e tutte le operazioni di history senza segnalare nessuna anomalia. Solo il verificatore, eseguito esplicitamente alla fine, rileva tutti i #gl("commit") non autorizzati in una singola analisi.

*Impatto: Alto.* L'assenza di RS07 e RS08 non produce nessun segnale operativo visibile — il sistema non avvisa mai autonomamente che qualcosa non va. La rilevazione dipende interamente dall'esecuzione periodica e sistematica del verificatore. In un contesto operativo reale dove il verificatore non viene eseguito ad ogni #gl("commit"), un dipendente non autorizzato può operare indisturbato per un periodo prolungato, accumulando modifiche non autorizzate che risultano crittograficamente valide.
...
#figure(caption: "Sintesi dei risultati — Scenario 2.")[
  #table(
    columns: (auto, 1fr, auto, auto, auto),
    table.header([*Tecnica*], [*Descrizione*], [*Verificatore*], [*Motore*], [*Impatto*]),
    [T1], [#gl("commit", capitalize: true) da chiave non autorizzata],  [ERR], [No], [Alto],
    [T2], [Author dichiarato diverso dal firmatario], [ERR], [No], [Alto],
    [T3], [Commit multipli senza segnale operativo],  [ERR], [No], [Alto],
  )
]

A differenza dello scenario 1, tutte le tecniche dello scenario 2 producono errori critici nel verificatore — la firma #gl("ssh", capitalize: true) è presente e il verificatore può confrontarla con il file `allowed_signers`. Tuttavia questo non riduce la gravità: il motore non blocca mai nessuno di questi #gl("commit") preventivamente. La protezione esiste solo a posteriori, tramite verifica esplicita. Il risultato chiave di questo scenario è che nella versione iniziale di #gl("rvc", capitalize: true) non esiste distinzione tra un dipendente autorizzato e uno non autorizzato — chiunque possieda una chiave #gl("ssh", capitalize: true) valida può operare su qualsiasi progetto senza restrizioni operative.

== Scenario 3 — Chiave privata di un dipendente compromessa

Il terzo scenario simula la compromissione della chiave privata #gl("ssh", capitalize: true) di un dipendente legittimo. L'attaccante può produrre #gl("commit") firmati crittograficamente identici a quelli del dipendente reale — la chiave è quella corretta e il firmatario è presente nel file `allowed_signers`. Questo è lo scenario più insidioso: la firma è valida, il firmatario è autorizzato, e non esiste nessun meccanismo automatico per distinguere un #gl("commit") fraudolento da uno legittimo.

Una nota sul verificatore: il verificatore attuale usa un file `allowed_signers` esterno e statico. Nel modello proposto ogni #gl("commit") porta il proprio `allowed_Dipendenti` interno al momento della firma — i #gl("commit") prodotti prima della revoca rimarrebbero validi perché la lista includeva la chiave al momento della firma. Per simulare entrambi i comportamenti vengono usati due file separati: `allowed_signers_con_chiave` che include la chiave compromessa, e `allowed_signers_senza_chiave` che non la include.

=== T1 — Commit fraudolento indistinguibile dal legittimo

L'attaccante produce un #gl("commit") firmato con la chiave rubata del dipendente legittimo. Il verificatore eseguito con `allowed_signers_con_chiave` segnala il #gl("commit") come completamente valido.
...
*Risultato.* Il #gl("commit") è accettato dal motore e risulta completamente valido al verificatore. Non esiste nessun indicatore automatico che permetta di distinguerlo da un #gl("commit") legittimo prodotto dal dipendente reale.
...
L'attaccante produce più #gl("commit") nell'intervallo tra la compromissione della chiave e la sua revoca, dimostrando concretamente la finestra di rischio descritta nel modello.
...
*Risultato.* Con la chiave presente nell'`allowed_signers` tutti i #gl("commit") — legittimi e fraudolenti — risultano validi. Dopo la rimozione della chiave, tutti risultano in errore indistintamente. Questo comportamento differisce dal modello proposto: con `allowed_Dipendenti` interno a ogni #gl("commit"), i #gl("commit") prodotti prima della revoca rimarrebbero validi perché la lista al momento della firma includeva la chiave. Il verificatore attuale non distingue tra "chiave valida al momento della firma" e "chiave valida ora".

*Impatto: Alto.* Durante la finestra di rischio l'attaccante può produrre qualsiasi numero di #gl("commit") fraudolenti che risultano indistinguibili da quelli legittimi. La dimensione di questa finestra dipende interamente dalla rapidità con cui la compromissione viene identificata e comunicata.

*Requisiti coinvolti:* RS09 — revoca efficace delle identità.

=== T3 — Propagazione nella catena dopo la revoca

Con la repository prodotta in T2, il verificatore viene eseguito con entrambi i file `allowed_signers` per confrontare i comportamenti e identificare i #gl("commit") sospetti.
...
*Risultato.* Con il verificatore attuale non è possibile distinguere automaticamente i #gl("commit") fraudolenti da quelli legittimi dopo la revoca — entrambi cambiano stato in blocco al cambio del file `allowed_signers`. La catena degli hash risulta intatta in entrambi i casi — i #gl("commit") fraudolenti non hanno alterato la struttura crittografica della storia.

*Impatto: Alto.* L'identificazione dei #gl("commit") fraudolenti richiede un'analisi manuale della history nel periodo sospetto. Nel modello proposto con `allowed_Dipendenti` interno questa distinzione sarebbe automatica — i #gl("commit") legittimi prodotti prima della revoca rimarrebbero validi mentre quelli fraudolenti prodotti con la stessa chiave sarebbero distinguibili per contenuto. Questa limitazione è la motivazione principale della scelta architetturale dell'`allowed_Dipendenti` versionato nel modello proposto.
...
Dopo aver prodotto un nuovo #gl("commit") firmato con la nuova chiave:
...
*Risultato.* La procedura di recovery è operativa — il nuovo #gl("commit") firmato con la nuova chiave risulta immediatamente valido. I #gl("commit") precedenti risultano tutti in errore indistintamente, senza distinzione tra legittimi e fraudolenti — limitazione già documentata in T3.

*Impatto.* La recovery non richiede interventi straordinari — è sufficiente aggiornare il file `allowed_signers`. La finestra di rischio si chiude dal #gl("commit") successivo alla revoca. La limitazione principale è che il verificatore attuale non distingue i #gl("commit") legittimi prodotti prima della revoca da quelli fraudolenti — nel modello proposto con `allowed_Dipendenti` interno questa distinzione sarebbe automatica.
...
#figure(caption: "Sintesi dei risultati — Scenario 3.")[
  #table(
    columns: (auto, 1fr, auto, auto, auto),
    table.header([*Tecnica*], [*Descrizione*], [*Verificatore*], [*Motore*], [*Impatto*]),
    [T1], [#gl("commit", capitalize: true) fraudolento indistinguibile], [OK (con chiave)],  [No], [Alto],
...
Lo scenario 3 è il più critico dell'intera simulazione — nessuna delle tecniche è rilevabile automaticamente durante la finestra di rischio. La compromissione di una chiave privata azzera tutte le garanzie crittografiche perché la chiave è l'unico indicatore di identità del sistema. Le uniche mitigazioni sono procedurali: rotazione periodica delle chiavi e revoca immediata alla scoperta della compromissione. Il modello proposto con `allowed_Dipendenti` interno a ogni #gl("commit") migliora la situazione post-revoca permettendo di distinguere automaticamente i #gl("commit") legittimi da quelli fraudolenti — ma non elimina la finestra di rischio, che rimane una limitazione strutturale di qualsiasi sistema basato su crittografia asimmetrica.
...
L'attaccante usa la chiave operativa dell'amministratore per produrre #gl("commit") su più progetti della #gl("repository"). Il motore accetta tutto senza distinzione. Il verificatore segnala tutti i #gl("commit") come validi perché la chiave è presente nel file `allowed_signers`.
...
*Risultato.* Il motore accetta tutti i #gl("commit") senza avvisi. Il verificatore li segnala tutti come validi — la chiave è quella corretta e il firmatario è nel file `allowed_signers`. Il risultato è tecnicamente identico allo scenario 3: la versione iniziale non distingue tra ruoli.
...
- Può produrre #gl("commit") amministrativi su qualsiasi progetto senza nessun vincolo di appartenenza
...
I quattro scenari documentano un quadro coerente delle vulnerabilità della versione iniziale di #gl("rvc", capitalize: true). Il sistema dispone di basi crittografiche solide — la catena degli hash e la firma #gl("ssh", capitalize: true) sono strumenti efficaci quando presenti — ma manca dei meccanismi organizzativi che trasformano queste garanzie tecniche in un modello di sicurezza praticabile in contesti multi-utente.
...
La firma #gl("ssh", capitalize: true) è il meccanismo di sicurezza più maturo della versione iniziale. T7 dimostra che qualsiasi modifica al file `.sig` di un #gl("commit") firmato produce un errore critico immediatamente rilevabile — la firma copre il file nella sua interezza e non lascia spazio a modifiche parziali. T3 dimostra analogamente che la catena degli hash rileva qualsiasi alterazione del contenuto ZIP senza ricalcolo.

Tuttavia la firma è opzionale nella versione iniziale, e questa opzionalità vanifica le protezioni che offre. T4 e T5 dimostrano che in assenza di firma è possibile riscrivere la storia di un progetto — modificando ZIP e ricalcolando gli hash in cascata — riducendo l'output del verificatore a soli warning, senza errori critici. La firma #gl("ssh", capitalize: true) obbligatoria è quindi una condizione necessaria per la sicurezza del sistema, non una funzionalità opzionale. Questa osservazione corrisponde direttamente al requisito RS06 identificato nel modello e classificato come obbligatorio.

=== Osservazione 3 — L'assenza di controllo delle identità è la vulnerabilità strutturale principale

Gli scenari 2, 3 e 4 convergono sulla stessa vulnerabilità di fondo: nella versione iniziale non esiste nessuna lista di autorizzati per progetto. Qualsiasi soggetto in possesso di una chiave #gl("ssh", capitalize: true) — autorizzata o meno, legittima o rubata — può produrre #gl("commit") validi su qualsiasi progetto. Il verificatore può rilevare le anomalie confrontando le firme con un file `allowed_signers` esterno, ma questo controllo è manuale e non integrato nel flusso operativo del motore.

La conseguenza più grave di questa assenza è documentata nello scenario 3: la compromissione di una chiave privata non produce nessun segnale automatico rilevabile — il verificatore con la chiave ancora presente nell'`allowed_signers` segnala tutti i #gl("commit") come validi, inclusi quelli fraudolenti. Solo la rimozione esplicita della chiave dall'`allowed_signers` permette di identificare le anomalie, ma a quel punto non è possibile distinguere automaticamente i #gl("commit") legittimi prodotti prima della revoca da quelli fraudolenti — entrambi risultano in errore indistintamente. Questa limitazione è la motivazione principale della scelta architetturale dell'`allowed_Dipendenti` versionato proposta nel modello, che risolve il problema mantenendo all'interno di ogni #gl("commit") la lista degli autorizzati valida al momento della firma.
...
#figure(caption: "Corrispondenza tra vulnerabilità osservate e requisiti del modello.")[
  #table(
    ...
    [Nessuna radice di fiducia — repository fasulla indistinguibile], [S1 T10], [RS05],
...
Tutti i requisiti classificati come assenti nell'analisi del divario del @cap:modello-sicurezza trovano una corrispondenza diretta in almeno una delle tecniche documentate in questo capitolo. I requisiti RS01 e RS02, classificati come parziali, sono confermati: la catena degli hash funziona correttamente quando i file non vengono sostituiti interamente, ma non costituisce una protezione sufficiente in assenza di firma obbligatoria. Questi risultati definiscono con precisione la priorità degli interventi descritti nel capitolo successivo.