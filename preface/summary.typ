...
\ \
Il lavoro ha previsto lo studio delle tecnologie crittografiche alla base del sistema — chiavi #gl("ssh", capitalize: true), firma-digitale e strumenti di cifratura — seguito dalla simulazione di scenari di attacco con diversi livelli di accesso. Per ciascuno scenario sono state individuate le vulnerabilità presenti, analizzate le possibili conseguenze e proposte contromisure concrete.
...
/ #link(<cap:analisi-dei-requisiti>)[Il quarto capitolo]: espone l'analisi del codice sorgente #gl("cpl", capitalize: true) di #gl("rvc", capitalize: true), descrive le vulnerabilità individuate e presenta il modello di minaccia costruito secondo il framework Mitre-attack;
