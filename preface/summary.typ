...
\ \
Il lavoro ha previsto lo studio delle tecnologie crittografiche alla base del sistema — chiavi #gl("ssh", capitalize: true), firma-digitale e strumenti di cifratura — seguito dalla definizione di un modello di sicurezza formale che identifica i requisiti di un sistema di versionamento distribuito sicuro, dalla progettazione di una gerarchia di fiducia e di livelli di sicurezza configurabili, e dalla simulazione di scenari di attacco con diversi livelli di accesso. Infine, sono stati implementati miglioramenti nel codice sorgente di #gl("rvc", capitalize: true) per colmare le vulnerabilità individuate e avvicinare il sistema al modello di sicurezza proposto.
\ \
